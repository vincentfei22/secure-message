...
export function ReactionModal({
    ...
    const [selected, setSelected] = useState<Reaction>(reactions[reactions.length - 1]);

    useEffect(() => {
        setSelected(
            reactions.find((r) => r.value === myReaction) || reactions[reactions.length - 1]
        );
    }, [myReaction]);

    const handleOnChange = async (e: Reaction) => {
        setSelected(e);
        try {
