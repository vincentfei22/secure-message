...
export default function Reader({ text, isEdited }: { text: string; isEdited: boolean; }) {
    ...
    const display = (text && isEdited && position)
        ? (!endWithPTag ? `${text}<p><span class="ql-size-small" style="color: rgb(136, 136, 136);"> (edited)</span></p>`
            : [text.slice(0, position), '<span class="ql-size-small" style="color: rgb(136, 136, 136);"> (edited)</span>', text.slice(position)].join("")
        ) : text;
    ...
    return (
      ...
      `} />
            <ReactQuill value={display} theme="bubble" readOnly ref={editorRef} className={classNames(/[!"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~A-Za-z0-9]/g.test(onlyText?.replace(" (edited)", "")) ? "" : "editor-has-only-emoji", "reader")} />
        </>
    );
}
