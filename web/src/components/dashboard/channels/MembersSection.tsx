...
const MemberItem = ({
    ...
    const isDefaultOrOwner = defaultChannel || owner;
    const displayLoading = loading ? <Spinner className="th-color-for" /> : null;
    const displayTrash = !loading ? <TrashIcon className="h-6 w-6 th-color-red" onClick={deleteMember} /> : null;

    return (
        <li className="px-8 py-2 flex justify-between items-center cursor-pointer group">
            <div className={classNames(isDefaultOrOwner ? "" : "group-hover:w-5/6", "flex items-center w-full")}>
                <img className="rounded mr-4 h-10 w-10" src={photoURL || `${process.env.PUBLIC_URL}/blank_user.png`} alt={id} />
                <div className="font-bold truncate th-color-for">
                    {member?.fullName}
                    {isMe && <span className="font-normal opacity-70 ml-1 th-color-for">(me)</span>}
                    {owner && <span className="font-normal opacity-70 ml-1 th-color-for"> - creator</span>}
                </div>
            </div>
            {!isDefaultOrOwner && <div className="opacity-0 group-hover:opacity-100">{displayLoading || displayTrash}</div>}
        </li>
    );
};
...
const MembersSection = () => {
    ...
    const displayMembers = useMemo(() => {
        return members.filter((mb: any) => channel?.members.includes(mb.objectId))
    ...
    return (
        <>
            ...
            <ul className="w-full overflow-y-scroll" style={{ height: "460px" }}>
                <li className="px-8 py-2 flex items-center cursor-pointer" onClick={() => setOpen(true)}>
