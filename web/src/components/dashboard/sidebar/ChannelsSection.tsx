...
const ChannelActionButton = () => {
    const baseClass = "w-full hidden group-hover:inline-flex sm:ml-3 justify-center items-center py-2 px-4 border border-transparent text-base font-bold rounded focus:outline-none focus:ring-4 focus:ring-blue-200 sm:w-auto sm:text-sm disabled:opacity-50 opacity-0 group-hover:opacity-100";
    if (channel?.members?.includes(user?.uid)) {
        const text = channel?.isArchived ? "Unarchive" : "New message";
        const url = channel?.isArchived ? `/channels/${channel?.objectId}/unarchive` : `/channels/${channel?.objectId}/members`;
        return <ModalButton isSubmitting={loading} text={text} onClick={() => handlePostData(url)} className={baseClass} />;
    } else if (!channel?.isArchived) {
        return <ModalButton isSubmitting={loading} onClick={() => handlePostData(`/channels/${channel?.objectId}/members`, { email: user?.email })} text="Join" className={baseClass} />;
    }
    return null;
};
