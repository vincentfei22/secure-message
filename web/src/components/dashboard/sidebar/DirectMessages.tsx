...
function DirectMessage({ dm }: { dm: any }) {
    ...
    const notifications = dm ? dm.lastMessageCounter - (detail?.lastRead || 0) : 0;
    ...
    const showStatus = () => {
        if (notifications > 0 && typingArray?.length > 0) {
            return (
                <div className="rounded flex items-centerjustify-center px-1 th-color-brwhite th-bg-red font-semibold text-xs">
                    <DotsHorizontalIcon className="h-4 w-4 animate-bounce" />
                </div>
            );
        } else if (notifications > 0) {
            return (
                <div className="text-xs rounded px-2 focus:outline-none th-color-brwhite th-bg-red font-semibold flex items-center justify-center">
                    {notifications}
                </div>
            );
        } else if (loading) {
            return <Spinner className="h-4 w-4 flex-shrink-0 m-1" />;
        } else if (typingArray?.length > 0) {
            return (
                <div className="rounded flex items-center justify-center px-2 th-color-brwhite font-semibold text-xs">
                    <DotsHorizontalIcon className="h-4 w-4 animate-bounce" />
                </div>
            );
        } else {
    ...
    return (
        <div className="relative py-1 flex items-center justify-between cursor-pointer focus:outline-none group">
            <div
                className="pl-8 flex items-center w-full focus:outline-none"
                role="button"
                tabIndex={0}
                onClick={() => navigate(`/dashboard/workspaces/${workspaceId}/dm/${dm?.objectId}`)}
            >
                <div className="relative mr-2 flex-shrink-0">
                    <div className="rounded-full h-3 w-3 absolute bottom-0 right-0 transform translate-x-1 translate-y-1 flex items-center justify-center">
                        <div className={classNames(isPresent ? "" : "border border-gray-400", "rounded-full h-2 w-2")} style={{ backgroundColor: isPresent ? "#94e864" : "transparent" }} />
                    </div>
                    <img src={photoURL || `${process.env.PUBLIC_URL}/blank_user.png`} alt="message" className="rounded h-5 w-5" />
                </div>
                <div className={classNames(notifications ? "font-semibold" : "", "truncate w-36")}>
                    {!isMe ? value?.displayName : `${value?.displayName} (me)`}
                </div>
            </div>
            <div className="mr-3">{showStatus()}</div>
        </div>
    );
}
...
function AddTeammates() {
    ...
    return (
        <div className="flex items-center px-8 cursor-pointer focus:outline-none pt-2" onClick={() => { setOpen(true); setSection("members"); }}>
