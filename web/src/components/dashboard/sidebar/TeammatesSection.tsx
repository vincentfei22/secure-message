...
function MemberItem({ id, owner, member }: MemberItemProps) {
    const { themeColors } = useTheme();
    const { user } = useUser();
    const { workspaceId } = useParams();
    const { setOpenCreateMessage, setCreateMessageSection: setSection } = useModal();
    ...
    return (
        <li className="px-8 py-2 flex justify-between items-center cursor-pointer group">
            <div className="flex items-center group-hover:w-4/6 w-full">
                ...
                >
                    {member?.fullName}
                    {id === user?.uid && <span className="font-normal opacity-70 ml-1" style={{ color: themeColors?.foreground }}>(me)</span>}
                    {owner && <span className="font-normal opacity-70 ml-1" style={{ color: themeColors?.foreground }}> - owner</span>}
                </div>
            </div>
            <ModalButton
                ...
                onClick={
                    openDirect
                        ? () => {
                            navigate(`/dashboard/workspaces/${workspaceId}/dm/${openDirect.objectId}`);
                            setOpenCreateMessage(false);
                        }
                        : newMessage
                }
...
export default function TeammatesSection() {
    const { setOpenInviteTeammates, setOpenCreateMessage } = useModal();
    const { workspaceId } = useParams();
